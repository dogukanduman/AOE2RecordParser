...
def parse_match(handle):
        ...
        try:
            ...
            if op_type is fast.Operation.SYNC:
                ...
                if op_data == last_viewlock:
                    continue
                viewlock = Viewlock(timedelta(milliseconds=timestamp), Position(*op_data),
    ...
    return Match(
        ...
        datetime.fromtimestamp(data['de']['timestamp']) if data['version'] is Version.DE and data['de'][
            'timestamp'] else None,
...
def serialize(obj):
    ...
    def impl(obj):
        ...
        if type(obj) is list:
            ...
            return {k: v for k, v in {f.name: impl(getattr(obj, f.name)) for f in dataclasses.fields(obj)}.items() if
                    v is not None}
